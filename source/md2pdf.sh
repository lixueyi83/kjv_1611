...
# make change and then commit
cc()    ( sed -i "s/\<$1\>/$2/g" *.md && git add . && git commit -m "$1 -> $2" )

grepexamples()
{
    grep -e "[^ ]*eth " -r . | cat -n
    grep -Ev "nest|lest|priest|guest" *.md | grep -e "[^ ]*est " -r . | cat -n
    grep -e '<sup>\*\*\(..\)\*\*</sup>' *.md            # grep '<sup>**12**</sup>'
}

sedexamples()
{
    sed 's/<sup>\*\*\(.\)\*\*<\/sup>/**^\1^**/g' old.md > new.md    # substitute '<sup>**1**</sup>' to '**^1^**'
    sed 's/<sup>\*\*\(..\)\*\*<\/sup>/**^\1^**/g' old.md > new.md   # substitute '<sup>**12**</sup>' to '**^12^**'

    sed -i 's/\*\*\*\(..\)\*\*\* /**^\1^**/g' *.md                  # substitute ***1*** to **^1^**
    sed -i 's/\*\*\*\(.\)\*\*\* /**^\1^**/g' *.md                   # substitute ***12*** to **^12^**

    sed -i 's/\*\*\*\(.\)\*\*\* /<sup>**\1**<\/sup>/g' *.md         # substitute ***1*** to **<sup>1</sup>
    sed -i 's/\*\*\*\(..\)\*\*\* /<sup>**\1**<\/sup>/g' *.md        # substitute ***12*** to **<sup>12</sup>
}
